...
{
#define MAX_WEEKS 100	// larger than the number of weeks in any data set. (each line represents data for one week)
#define MAX_WEEKS2 1000	// larger than the number of weeks in any data set. (each line represents data for one week)
...
char *file;
char *file_name="data";      //CK// name for inputing the observational data
char *file_name2="expdata";  //CK// name for inputing the experimental data
char *file_name3="weather";  //CK// name for inputing the weather data
char *file_type=".txt";
...
//printf("within weather data sets \n");
	...
	while (fscanf(ftp_data,"%d %lf %lf %lf %lf %lf %lf %lf \n",&Ddata2[i],&Rain[i],&MaxT[i],&MinT[i],&AveT[i],&MaxRH[i],&MinRH[i],&AveRH[i])!= EOF)			{
		...
		Params->WDATA[j][i][0]=Ddata2[i]; Params->WDATA[j][i][1]=Rain[i]; Params->WDATA[j][i][2]=MaxT[i]; Params->WDATA[j][i][3]=MinT[i]; //TEMP
